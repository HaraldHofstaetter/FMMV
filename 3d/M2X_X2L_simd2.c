...
static void CMX_simd2(FmmvHandle *FMMV, _FLOAT_ *x, _FLOAT_ *y)
{
	int p = FMMV->pM;
	...
	k = p+1;
	gemv_trans_simd2(k, s_eps, V, s_eps, xp, yp);
	xp += 2*k;
	yp += 2*s_eps;
        V += s_eps*k;
...
static void CXL_simd2(FmmvHandle *FMMV, _FLOAT_ *x, _FLOAT_ *y, int reduced)
{
	int p = FMMV->pL;
	int s_eps = FMMV->s_eps;
	_FLOAT_ *V;
	...
	k = p+1;
	gemv_simd2(k, s_eps, V, s_eps, xp, yp);
	xp += 2*s_eps;
	yp += 2*k;
        V += s_eps*k;
...
static void CXL_minus_simd2(FmmvHandle *FMMV,  _FLOAT_ *x, _FLOAT_ *y, int reduced)
{
	int p = FMMV->pL;
	int s_eps = FMMV->s_eps;
	_FLOAT_*V;
	...
	if (p%2) {
		k = (p+1)/2;
		gemv_simd2(k, s_eps, V+s_eps, 2*s_eps, xp, yp);
		xp += 2*s_eps; yp += 2*k;
                V += s_eps*kk; kk--; /****/
		for (k=p/2; k>=1; k--) {
			gemv_simd2(k+1, s_eps, V, 2*s_eps, xp, yp);
			xp += 2*s_eps; yp += 2*(k+1);
			gemv_simd2(k , s_eps, V+s_eps, 2*s_eps, xp, yp);
			xp += 2*s_eps; yp += 2*k;
                        V += s_eps*kk; kk--; /****/
			gemv_simd2(k, s_eps, V+s_eps, 2*s_eps, xp, yp);
			xp += 2*s_eps; yp += 2*k;
			gemv_simd2(k, s_eps, V, 2*s_eps, xp, yp);
			xp += 2*s_eps; yp += 2*k;
                        V += s_eps*kk; kk--; /****/
		}
	...
	gemv_simd2(1 , s_eps, V, 2*s_eps, xp, yp);

}



static void CXL_plus_simd2(FmmvHandle *FMMV, _FLOAT_ *x, _FLOAT_ *y, int reduced)
{
	int p = FMMV->pL;
	int s_eps = FMMV->s_eps;
	_FLOAT_ *V;
...
static void CMX_minus_simd2(FmmvHandle *FMMV,  _FLOAT_ *x, _FLOAT_ *y)
{
	int p = FMMV->pM;
	int s_eps = FMMV->s_eps;
	_FLOAT_ *V = FMMV->CMX_coeffs;
	...
	if (p%2) {
		k = (p+1)/2;
		gemv_trans_simd2(k, s_eps, V+s_eps, 2*s_eps, xp, yp);
		yp += 2*s_eps; xp += 2*k;
                V += s_eps*kk; kk--; /****/
		for (k=p/2; k>=1; k--) {
			gemv_trans_simd2(k+1, s_eps, V, 2*s_eps, xp, yp);
			yp += 2*s_eps; xp += 2*(k+1);
			gemv_trans_simd2(k , s_eps, V+s_eps, 2*s_eps, xp, yp);
			yp += 2*s_eps; xp += 2*k;
                        V += s_eps*kk; kk--; /****/
			gemv_trans_simd2(k, s_eps, V+s_eps, 2*s_eps, xp, yp);
			yp += 2*s_eps; xp += 2*k;
			gemv_trans_simd2(k, s_eps, V, 2*s_eps, xp, yp);
			yp += 2*s_eps; xp += 2*k;
                        V += s_eps*kk; kk--; /****/
		}
		gemv_trans_simd2(1, s_eps, V, 2*s_eps, xp, yp);
		memset(yp+2*s_eps, 0, 2*s_eps*sizeof(_FLOAT_));
		...
		memset(yp, 0, 2*s_eps*sizeof(_FLOAT_));
		gemv_trans_simd2(1, s_eps, V, 2*s_eps, xp, yp+2*s_eps);
	}	
...
static void CMX_plus_simd2(FmmvHandle *FMMV, _FLOAT_ *x, _FLOAT_ *y)
{
	int p = FMMV->pM;
	int s_eps = FMMV->s_eps;
	_FLOAT_ *V = FMMV->CMX_coeffs;
