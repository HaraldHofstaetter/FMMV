from fmmv_setup import 	name,additional_options, additional_statistics, float_types,dim, charge_kinds, pot_kinds, charge_pot_combs, charge_combs, pot_combs, eval_direct_number_of_accuracies;
...
struct FmmvOptions fmmvGetDefaultOptions(void);
	...
	for float_type in float_types:	
		print "void fmmv%s(" % Aa(name[float_type])
	...
	for float_type in float_types:	
		print "void fmmv%s_initialize(" % Aa(name[float_type])
	...
	for float_type in float_types:	
		print "void fmmv%s_evaluate(" % Aa(name[float_type])
		print "\tvoid* fmmvHandle,"
		print_charge_args(charge_kinds, float_type)
		print_pot_args(pot_kinds, float_type)
		print "\tstruct FmmvStatistics *statistics, /* can be NULL */"
	...
	for float_type in float_types:
		print "void fmmv%s_finalize(" % Aa(name[float_type])
		print "\tvoid* fmmvHandle,"
		print "\tstruct FmmvStatistics *statistics, /* can be NULL */"
                print "\tchar **errorMessage);"
		print
	...
	for suf in [""]:
	    for float_type in float_types:	
		print "void fmmv%s_direct%s(" % (Aa(name[float_type]), suf)
		print "\tunsigned int NParticles, "
		print "\t%s particles[][%i]," % (float_type, dim)
		print_charge_args(charge_kinds, float_type)
		print "\tunsigned int NTargets, "
...
def gen_fmmv_all_in_one():
	...
	for float_type in float_types:
		...
		i += 1
		print "fmmv%s" % Aa(name[float_type])
	print "#endif"
	print "(\tunsigned int NParticles, "
	...
	for float_type in float_types:
		if i>0: print "#elif (FMM_PRECISION==%i)" % i
		i += 1
		print "\tfmmv%s_initialize" % Aa(name[float_type])
	print "#endif"
	print "\t(\t(void**) &FMMV,"
	...
	for float_type in float_types:
		if i>0: print "#elif (FMM_PRECISION==%i)" % i
		i += 1
		print "\tfmmv%s_evaluate" % Aa(name[float_type]) 
	print "#endif"
	print "\t(\t(void*) FMMV,"
	print "\t\t",
	charge_kinds_sorted = range(len(charge_kinds))
	...
	for float_type in float_types:
		if i>0: print "#elif (FMM_PRECISION==%i)" % i
		i += 1
		print "\tfmmv%s_finalize((void*) FMMV, statistics, &err);" %  Aa(name[float_type])
	print "#endif"
	print "\tif (err) goto _err;"
	print
...
def gen_fmmv_finalize():
	...
	for float_type in float_types:
		if i>0: print "#elif (FMM_PRECISION==%i)" % i
		i += 1
		print "fmmv%s_finalize" % Aa(name[float_type])
	print "#endif"
	print "(\tvoid* fh,"
	print "\tstruct FmmvStatistics *statistics, /* can be NULL */"
	print "\tchar **errorMessage)"
...
def gen_direct_method():
	...
	for float_type in float_types:
		if i>0: print "#elif (FMM_PRECISION==%i)" % i
		i += 1
		print "fmmv%s_direct" % Aa(name[float_type])
	print "#endif"
	print "(\tunsigned int NParticles, "
	print "\t%s particles[][%i]," % ("_FLOAT_", dim)
	print_charge_args(charge_kinds, "_FLOAT_")
	...
	int typeTargets = 0;
        """
        print "\tif (accuracy<0) {"
	print "\t\tdirect_method_xp(NParticles, particles,"
